...
import { View, StyleSheet, TextInput, FlatList, ActivityIndicator, Text, Image, TouchableOpacity } from 'react-native'
...
export default function AppSearch() {
    ...
    const searchNftType = (txt) => {
        if (txt !== '') {
            setloading(true);
            dispatch(searchNFT(txt)).then((response) => {
    ...
    return (
        <View style={styles.container}>
            ...
            {(loading || searchData?.length) ? <View style={styles.listContainer}>
                {loading ?
