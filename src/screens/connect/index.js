  ...
  return (
    <TouchableOpacity
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          props.onDisconnect && props.onDisconnect(item, details?.name)
        }>
            ...
            if (response.type == 'newconnectionrequest') {
              ...
              if (response.data.isAppApproved == 'true') {
                ...
                axios
                  .post(url, body)
                  .then(response => {
                    let signature = getSig(
                      response.data?.data?.nonce,
                      wallet.privateKey,
                    );
                    axios
                      .post(urlXanalia, bodyXanalia)
                      .then(responseXanalia => {
                        let signatureXanalia = getSig(
                          responseXanalia.data?.data,
                          wallet.privateKey,
                        );
                        let _data = {
                          type: 'sig',
    ...
    <AppBackground>
      ...
      <View style={styles.container}>
        <TextView style={styles.heading}>
          {translate('wallet.common.connectInfo')}
          {'\n'}
          {translate('wallet.common.connectInfo2')}
        </TextView>
        ...
        <AppButton
          label={translate('wallet.common.connect')}
