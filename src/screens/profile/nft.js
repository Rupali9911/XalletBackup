...
import {authUserNFTList} from '../../store/actions/myNFTaction';
...
const NFT = ({route}) => {
  ...
  const getCreatedNftList = (page, refresh) => {
    dispatch(authUserNFTList(page, id, refresh));
    ...
    //         } else {
    //           setNftCreatedList([...nftCreatedList, ...res.data.data]);
    //         }
    //       } else {
    //         setStopMoreLoading(true);
    //       }
      ...
      {MyNFTReducer.myNftListLoading ? (
        ...
        <FlatList
          ...
          onEndReached={() => {
            if (!stopMoreLoading) {
              if (toggle == 'created') {
                let num = nftCreatedPage + 1;
                setNftCreatedPage(num);
                getCreatedNftList(num);
              } else {
